...
for b=1:T_bal
    for r=1:T_rounds
        ...
        for d=1:T_d
            texto = ['D = ' num2str(D(d)) '(',num2str(d),'/',num2str(T_d),'), Round:' num2str(r),'/',num2str(T_rounds),', bal = ' num2str(bal(b)),' (',num2str(b),'/',num2str(T_bal),').' ];
            options.B = 10;
            options.Dist = 'norm';
            options.MaxK = D(d);
            options.MinK = D(d);
            options.Tries = 20;
            disp(texto)
            [R(d,r,b),MC(:,:,d,r,b)] = GAP_actions(trajectories,test_samples,training_samples,options);
        end
    end
end
